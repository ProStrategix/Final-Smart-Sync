...
import {goTo, initializeNotices, pushMessage, /*pushMessageImg,*/ updateStatus, pause, populateClassificationResults} from 'public/stateManagement.js'
...
import { uploadAccessCsv, processCsv,  splitNormalizedRowsIntoStructuredData } from 'public/dataManagement.js'
...
let messages = []
// let imageMessages = [] // COMMENTED OUT - Image management not yet rebuilt
let mainLog 
let entries = []
const loc = "widget.js"
// let finalProducts = []
...
$w('#uploadCsvButton').onChange((event) => {
    $w('#uploadCsvButton').uploadFiles()
        ...
        .then(async (normalized) => {
            ({parsedData, imageResults} = await splitNormalizedRowsIntoStructuredData(normalized.normalizedRows, messages))
            
            // Populate the classification repeater with results
            if (imageResults && imageResults.imgs && imageResults.imgs.length > 0) {
                console.log('Populating classification repeater with', imageResults.imgs.length, 'items');
                await populateClassificationResults(imageResults.imgs);
                postEntry(`Classification results displayed: ${imageResults.imgs.length} images classified`, 'success', loc, null);
            } else {
                console.log('No classification results to display');
                postEntry('No classification results to display', 'warning', loc, null);
            }
